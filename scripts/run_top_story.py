...
"""
run_top_story.py
Wrapper for generate_top_story.py that uses OpenAI GPT-4.1-mini when GROQ_API_KEY
is not available. All prompts, port registry, and logic are identical.
...
# ── LLM story writer ──────────────────────────────────────────────────────────
def write_story(top: dict, runner_up, group_label: str):
    ...
    if not headline or len(headline) < 5:
        raise ValueError(f"LLM returned empty or too-short headline: {repr(headline)}")
    if not paragraph or len(paragraph) < 20:
        raise ValueError(f"LLM returned empty or too-short paragraph: {repr(paragraph)}")

    return headline, paragraph

# ── Data-driven fallback ──────────────────────────────────────────────────────
def build_fallback(top: dict, group_label: str):
    max_wind_kt = round(max((ms_to_kt(v) for v in top["daily_wind_max"] if v is not None), default=0))
    max_wave    = round(max((v for v in top["daily_wave_max"] if v is not None), default=0), 1)
    max_rain    = round(max((v for v in top["daily_rain"] if v is not None), default=0))
    max_period  = round(max((v for v in top["daily_swell_period"] if v is not None), default=0))
    ...
    paragraph = f"{sentence1} {rain_str}"
    return headline, paragraph
...
async def main():
    ...
    print("Generating Caribbean story...")
    try:
    ...
    print("Pausing 5 seconds before Mediterranean story call...")
    time.sleep(5)

    print("Generating Mediterranean story...")
    try:
