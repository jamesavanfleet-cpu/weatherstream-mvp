    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(12,1fr)", gap: 3 }}>
      {months.map((m, i) => (
        <div
          ...
          title={`${displayText(m.month)}: ${displayText("Score")} ${m.score}`}
          style={{
    ...
    >
      ...
      <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid #1e3a5f", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
        <div>
          ...
          <div data-no-localize="true" style={{ fontSize: 11, fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em", marginTop: 3 }}>{displayText(port.region)}</div>
        </div>
      ...
      <div style={{ padding: "14px 20px", borderBottom: "1px solid #1e3a5f" }}>
        <div data-no-localize="true" style={{ fontSize: 11, fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>{displayText("Weather Score by Month")}</div>
      ...
      <div style={{ padding: "14px 20px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          ...
          <div key={s.label} style={{ textAlign: "center" }}>
            ...
            <div data-no-localize="true" style={{ fontSize: 10, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", marginTop: 2 }}>{displayText(s.label)}</div>
          </div>
        ))}
      ...
      {/* Best month */}
      <div data-no-localize="true" style={{ padding: "0 20px 16px", display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ fontSize: 12, color: "#94a3b8" }}>{displayText("Best month:")}</span>
        <span style={{ fontSize: 13, fontWeight: 700, color: "#f59e0b", background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 6, padding: "3px 10px" }}>
          {displayText(best.month)} ({Math.round(best.score)})
        </span>
      </div>
    </div>
  );
      ...
      <div style={{ background: "#1a2235", border: "1px solid #1e3a5f", borderRadius: 16, maxWidth: 760, width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ padding: "24px 28px 18px", borderBottom: "1px solid #1e3a5f", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, position: "sticky", top: 0, background: "#1a2235", zIndex: 1 }}>
          <div>
            ...
            <div data-no-localize="true" style={{ fontSize: 13, color: "#94a3b8", marginTop: 4 }}>{displayText(port.region)} - {displayText("30-Year Climate Analysis (NASA POWER 1994-2023)")}</div>
          </div>
        ...
        <div style={{ padding: "24px 28px" }}>
          <div data-no-localize="true" style={{ fontSize: 13, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 12 }}>{displayText("Monthly Weather Score")}</div>
            ...
            {port.months.map(m => {
              ...
              return (
                <div key={m.month} style={{ background: isBest ? "rgba(245,158,11,0.08)" : "#111827", border: `1px solid ${isBest ? "#f59e0b" : "#1e3a5f"}`, borderRadius: 10, padding: 14, textAlign: "center" } as React.CSSProperties}>
                  <div data-no-localize="true" style={{ fontSize: 13, fontWeight: 700, marginBottom: 8 }}>{displayText(m.month)}{isBest ? " \u2605" : ""}</div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                      ...
                      <div data-no-localize="true" key={s.label} style={{ fontSize: 11, color: "#94a3b8" }}>
          ...
          <div data-no-localize="true" style={{ fontSize: 13, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 12 }}>{displayText("Best Month Breakdown")} ({displayText(best.month)})</div>
            ...
            {[
              ...
              return (
                <div key={p.label} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div data-no-localize="true" style={{ fontSize: 12, color: "#94a3b8", width: 100, flexShrink: 0 }}>{displayText(p.label)}</div>
          ...
          <div data-no-localize="true" style={{ fontSize: 12, color: "#94a3b8", lineHeight: 1.6 }}>
            {displayText("Score methodology: Rain probability (30%), Wind speed (25%), Cloud cover (20%), Temperature comfort (25%).")} {displayText("Scores are based on 30 years of NASA POWER reanalysis data (1994-2023). Higher scores indicate better cruising weather conditions.")}
          </div>
        </div>
      </div>
    </div>
...
export default function JamesPicks() {
  ...
  const monthLabel = selectedMonth !== null
    ? displayText(MONTHS_FULL[selectedMonth])
    : displayText("Best Month");
    ...
    <div style={S.page}>
      ...
      <div style={S.hero}>
        ...
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(56,189,248,0.12)", border: "1px solid rgba(56,189,248,0.3)", borderRadius: 999, padding: "6px 16px", fontSize: 12, fontWeight: 600, color: "#38bdf8", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 20 }}>
          ...
          <span data-no-localize="true">{displayText("Weather Intelligence by James Van Fleet")}</span>
        </div>
        <h1 style={{ fontSize: "clamp(28px,5vw,48px)", fontWeight: 800, letterSpacing: "-0.02em", lineHeight: 1.1, marginBottom: 10 }}>
          <span data-no-localize="true">{displayText("James' Picks for")} </span><span data-no-localize="true" style={{ color: "#38bdf8" }}>{displayText("Best Weather")}</span>
        </h1>
        <p style={{ fontSize: 15, color: "#e2e8f0", maxWidth: 580, margin: "0 auto 10px", lineHeight: 1.5 }}>
          <span data-no-localize="true">{displayText("For planning what time of year and where to go on your next Cruise")}</span>
        </p>
        <p style={{ fontSize: 16, color: "#94a3b8", maxWidth: 580, margin: "0 auto 28px", lineHeight: 1.6 }}>
          <span data-no-localize="true">{displayText("30 years of NASA POWER reanalysis data (1994-2023) analyzed across every cruise port. Scored on rain probability, wind, cloud cover, and temperature comfort -- so you know exactly when to go where.")}</span>
        </p>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 8, padding: "8px 16px", fontSize: 13, color: "#f59e0b" }}>
          ...
          <span data-no-localize="true">{displayText("Based on NASA POWER reanalysis -- 30 years of observed climate data")}</span>
        </div>
      </div>

      {/* Controls */}
      <div style={S.controls}>
        <button data-no-localize="true" style={S.backBtn} onClick={() => navigate("/")}>&#8592; {displayText("Back")}</button>
        <span data-no-localize="true" style={{ fontSize: 12, fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.06em", whiteSpace: "nowrap" }}>{displayText("Filter:")}</span>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, flex: 1 }}>
          <button data-no-localize="true" style={S.filterBtn(activeRegion === "all")} onClick={() => setActiveRegion("all")}>{displayText("All Regions")}</button>
          {REGIONS.map(r => (
            <button data-no-localize="true" key={r} style={S.filterBtn(activeRegion === r)} onClick={() => setActiveRegion(r)}>{displayText(r === "US Homeport" ? "US Homeports" : r)}</button>
          ))}
        </div>
        <select style={S.select} value={selectedMonth ?? "best"} onChange={e => setSelectedMonth(e.target.value === "best" ? null : parseInt(e.target.value))}>
          <option value="best">{displayText("Best Month")}</option>
          {MONTHS_FULL.map((m, i) => <option key={m} value={i}>{displayText(m)}</option>)}
        </select>
        <input data-no-localize="true" style={S.input} type="text" placeholder={displayText("Search port...")} value={search} onChange={e => setSearch(e.target.value)} />
        <div style={{ display: "flex", gap: 4, background: "#1a2235", border: "1px solid #1e3a5f", borderRadius: 8, padding: 3 }}>
          {(["cards","table","heatmap"] as const).map(v => (
            <button key={v} style={S.viewBtn(view === v)} onClick={() => setView(v)}>
              <span data-no-localize="true">{displayText(v.charAt(0).toUpperCase() + v.slice(1))}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={S.colorKey}>
        <span data-no-localize="true" style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.07em", whiteSpace: "nowrap" }}>{displayText("Score Key:")}</span>
        {[
          ...
          <span key={k.label} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, whiteSpace: "nowrap" }}>
            ...
            <span data-no-localize="true">{displayText(k.label)}</span>
          </span>
        ))}

        <span data-no-localize="true" style={{ fontSize: 11, color: "#94a3b8", fontStyle: "italic", marginLeft: "auto" }}>{displayText("Scored on rain, wind, cloud cover & temperature comfort")}</span>
      </div>
      ...
      <div style={S.main}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
          <div style={{ fontSize: 14, color: "#94a3b8" }}>
            <span data-no-localize="true">{displayText("Showing")} <strong style={{ color: "#e2e8f0" }}>{sorted.length}</strong> {displayText("ports")} - {monthLabel}</span>
          </div>
          <div data-no-localize="true" style={{ fontSize: 12, color: "#94a3b8" }}>{displayText("Click any port for full monthly breakdown")}</div>
        </div>

        {loading && (
          <div data-no-localize="true" style={{ textAlign: "center", padding: "80px 24px", color: "#94a3b8" }}>{displayText("Loading climate data...")}</div>
        )}
        {error && (
          <div data-no-localize="true" style={{ textAlign: "center", padding: "80px 24px", color: "#f97316" }}>{displayText("Error:")} {error}</div>
        )}
              ...
              <thead>
                <tr>
                  {[
                    ...
                    <th data-no-localize="true" key={h.col} onClick={() => handleSort(h.col)} style={{ background: "#111827", borderBottom: "2px solid #1e3a5f", color: "#94a3b8", fontSize: 11, fontWeight: 600, letterSpacing: "0.06em", padding: "10px 12px", textAlign: "left", textTransform: "uppercase", whiteSpace: "nowrap", cursor: "pointer", userSelect: "none" }}>
                      {displayText(h.label)} {sortCol === h.col ? (sortDir === -1 ? "v" : "^") : ""}
                    </th>
                  ))}
                </tr>
              </thead>
                ...
                {sorted.map(port => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td data-no-localize="true" style={{ borderBottom: "1px solid rgba(30,58,95,0.5)", padding: "10px 12px", color: "#94a3b8" }}>{displayText(port.region)}</td>
                      ...
                      <td data-no-localize="true" style={{ borderBottom: "1px solid rgba(30,58,95,0.5)", padding: "10px 12px", color: "#f59e0b", fontWeight: 600 }}>{displayText(best.month)}</td>
        ...
        {!loading && !error && view === "heatmap" && (
          <div style={{ overflowX: "auto" }}>
            <table style={{ borderCollapse: "collapse", fontSize: 12, minWidth: 900, width: "100%" }}>
              <thead>
                <tr>
                  <th data-no-localize="true" style={{ background: "#111827", border: "1px solid #1e3a5f", color: "#94a3b8", fontSize: 11, fontWeight: 600, padding: "8px 12px", textAlign: "left", minWidth: 160 }}>{displayText("Port")}</th>
                  {MONTHS_FULL.map(m => (
                    <th data-no-localize="true" key={m} style={{ background: "#111827", border: "1px solid #1e3a5f", color: "#94a3b8", fontSize: 11, fontWeight: 600, padding: "8px 10px", textAlign: "center", whiteSpace: "nowrap" }}>{displayText(m)}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {sorted.map(port => (
                  <tr key={port.name}>
                    <td onClick={() => setModalPort(port)} style={{ border: "1px solid rgba(30,58,95,0.4)", padding: "6px 12px", fontWeight: 500, fontSize: 12, color: "#e2e8f0", whiteSpace: "nowrap", background: "#111827", cursor: "pointer" }}>
                      {port.name}<br /><span data-no-localize="true" style={{ fontSize: 10, color: "#94a3b8" }}>{displayText(port.region)}</span>
                    </td>
                    {port.months.map((m, i) => (
                      <td data-no-localize="true" key={m.month} style={{ border: "1px solid rgba(30,58,95,0.4)", padding: "6px 4px", textAlign: "center", fontWeight: 700, fontSize: 11, background: getHeatColor(port.name, m.score), color: "#fff", outline: selectedMonth === i ? "2px solid #fff" : undefined, outlineOffset: selectedMonth === i ? -2 : undefined, cursor: "default" }} title={`${port.name} ${displayText(m.month)}: ${displayText("Score")} ${m.score}, ${displayText("Rain")} ${m.rain_prob}%, ${displayText("Wind")} ${m.wind_kt}kt`}>
                        {Math.round(m.score)}
                      </td>
                    ))}
                  </tr>
