...
interface CruiseData {
  generated_at: string;
  port_coords: Record<string, PortCoords>;
...
// Get port arrival date based on departure date and port index
function getPortDate(departureDate: string, portIndex: number): string {
  const d = new Date(departureDate + "T12:00:00Z");
  d.setDate(d.getDate() + portIndex + 1); // Day 1 = first port after departure
  return d.toISOString().slice(0, 10);
}

// Fetch weather for a port on a specific date
async function fetchPortWeather(lat: number, lon: number, date: string): Promise<Partial<PortForecast>> {
  try {
    ...
    if (idx === -1) return { condition: "No data", loading: false, error: false };
...
export default function CruiseFinder({ isMetric }: CruiseFinderProps) {
  ...
  // Get available ships for selected line
  const availableShips = cruiseData?.cruise_lines
    .find(cl => cl.id === selectedLine)?.ships ?? [];

  // Get available dates for selected ship
  const availableDates = availableShips
    .find(s => s.name === selectedShip)?.itineraries ?? [];
  ...
  // Fetch forecasts when itinerary is selected
  useEffect(() => {
    ...
    const initial: PortForecast[] = ports.map((port, idx) => ({
      port,
      date: getPortDate(selectedItinerary.departure_date, idx),
      lat: cruiseData.port_coords[port]?.lat ?? 0,
      lon: cruiseData.port_coords[port]?.lon ?? 0,
      tempF: null,
      tempC: null,
      windKt: null,
      windDir: "--",
    ...
    // Fetch all port forecasts
    Promise.all(
      initial.map(async (pf, idx) => {
        if (!pf.lat && !pf.lon) {
          return { ...pf, loading: false, error: false, condition: "Location unknown" };
        }
  ...
  if (dataError) {
    return (
      <div className="text-center py-8 text-white/40 text-sm">
        Cruise itinerary data unavailable. Please check back shortly.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            >
              ...
              {cruiseData?.cruise_lines.map(cl => (
                <option key={cl.id} value={cl.id} className="bg-[#0a0f1a]">{cl.display}</option>
              ))}
            </select>
          ...
          <div className="relative">
            ...
            >
              <option value="" disabled className="bg-[#0a0f1a]">
                {selectedLine ? "Select ship..." : "Select cruise line first"}
              </option>
              {availableShips.map(s => (
                <option key={s.name} value={s.name} className="bg-[#0a0f1a]">{s.name}</option>
              ))}
            </select>
            <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40 pointer-events-none" />
          </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            >
              <option value="" disabled className="bg-[#0a0f1a]">
                {selectedShip ? "Select departure date..." : "Select ship first"}
              </option>
              {availableDates.map(i => (
                <option key={i.departure_date} value={i.departure_date} className="bg-[#0a0f1a]">
                  {formatDate(i.departure_date)} {i.duration_days ? `(${i.duration_days} days)` : ""} -- {i.departure_port}
                </option>
              ))}
            </select>
            <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40 pointer-events-none" />
      ...
      {/* Itinerary summary */}
      {selectedItinerary && (
        <div className="bg-white/5 border border-white/10 rounded-lg px-4 py-3 flex flex-wrap items-center gap-3 text-sm">
          <span className="text-cyan-400 font-semibold">{selectedShip}</span>
          <span className="text-white/30">|</span>
          <span className="text-white/70">{selectedItinerary.description}</span>
        </div>
      )}
      ...
      {portForecasts.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-white/70 text-sm font-semibold tracking-wide uppercase">Port-by-Port Forecast</span>
            <span className="text-white/30 text-xs ml-1">-- hover each port for full details</span>
          </div>

          {/* Port tabs */}
          <div className="flex flex-wrap gap-2 mb-4">
            {portForecasts.map((pf, idx) => (
              <button
                key={idx}
                onMouseEnter={() => setHoveredPort(pf.port)}
                onMouseLeave={() => setHoveredPort(null)}
                onClick={() => setHoveredPort(hoveredPort === pf.port ? null : pf.port)}
              ...
              >
                <span className="mr-1">{conditionIcon(pf.condition)}</span>
                {pf.port}
          ...
          {/* Hover detail panel */}
          {hoveredPort && (() => {
            const pf = portForecasts.find(p => p.port === hoveredPort);
            if (!pf) return null;
            return (
              <div className="bg-white/5 border border-cyan-400/30 rounded-xl p-5 animate-in fade-in duration-200">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-white/50 text-sm mt-0.5">{formatDate(pf.date)}</p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div className="bg-white/5 rounded-lg p-3">
                      ...
                      <div className="text-white font-bold text-xl">
                        {isMetric
                          ? (pf.tempC !== null ? `${pf.tempC}°C` : "--")
                          : (pf.tempF !== null ? `${pf.tempF}°F` : "--")}
                      </div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3">
                      <div className="flex items-center gap-1.5 text-white/40 text-xs mb-1">
                      ...
                      <div className="text-white font-bold text-xl">
                        {pf.windKt !== null
                          ? (isMetric ? `${Math.round(pf.windKt * 1.852)} km/h` : `${pf.windKt} kt`)
                          : "--"}
                      </div>
                    ...
                    <div className="bg-white/5 rounded-lg p-3">
                      ...
                      <div className="text-white font-bold text-xl">
                        {pf.precip !== null
                          ? (isMetric ? `${pf.precip} mm` : `${(pf.precip / 25.4).toFixed(2)}"`)
                          : "--"}
                      </div>
                    </div>
                    <div className="bg-white/5 rounded-lg p-3">
          ...
          {/* Default state - no port hovered */}
          {!hoveredPort && (
            <div className="text-center py-6 text-white/30 text-sm">
              Hover or tap any port above to see the 5-day forecast
            </div>
          )}
        </div>
      )}
